...
7. [Шаг 5 — Проверка ветки в remote](#шаг-5--проверка-ветки-в-remote)
...
- если аргумент просит **обновить описание** существующего MR — сразу переходи в Шаг 2 → Шаг 6 → обновление, минуя Шаг 3 (вопрос про готовность ветки), если MR уже существует;
...
flowchart TD
    ...
    E --> F[Шаг 5: Проверить ветку в remote\nне пушить]
    F --> F1{Ветка есть в remote?}
    F1 -->|Да| G
    F1 -->|Нет| STOP3([🛑 Стоп: запушь ветку вручную])
    G --> H{MR уже открыт?}
...
```bash
# 0. Рабочий CLI — всегда dp glab
GLAB="dp glab"
dp glab auth status >/dev/null 2>&1 || { echo "Ошибка: dp glab не авторизован." && exit 1; }
...
Если `dp glab` не авторизован (`exit 1`) — не останавливай workflow молча: спроси пользователя через `ask_user_question`, как продолжить (например, предложи выполнить `dp auth login`), дождись ответа и после авторизации повтори Шаг 1.

**Стоп-условия:**
- не в git-репозитории
- `dp glab` не аутентифицирован (и пользователь не авторизовал его)
- `current_branch` совпадает с `target_branch`
- не удалось определить `target_branch`

Во всех последующих шагах используй `$GLAB` (всегда `dp glab`).

Формат `--repo` — `<host>/<namespace>/<repo>` (из `git remote get-url origin`). `dp glab` принимает его напрямую, проверка не требуется.
